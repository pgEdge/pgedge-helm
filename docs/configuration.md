...
By default, this chart supports all extensions included in the standard flavor of the [pgEdge Enterprise Postgres Image](https://github.com/pgedge/postgres-images?tab=readme-ov-file#standard-images). You can add these extensions to your database by running `CREATE EXTENSION`, with no additional required configuration.

Extensions which require placement in `shared_preload_libraries` are handled by the default `values.yaml` configuration. If you need to customize which extensions are loaded at runtime, you can override `shared_preload_libraries` in your `values.yaml`.
...
| pgEdge.initSpockImageName | string | `"ghcr.io/pgedge/pgedge-helm-utils:v0.0.4"` | Docker image for the init-spock job. This image contains Python and required dependencies for the job to run. This image is versioned alongside the Helm chart. |
